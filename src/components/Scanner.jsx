...
export default function Scanner({ user, onLogout }) {
  ...
  const handleScan = async () => {
    if (!styleNumber.trim()) {
      toast.error('Style number daalen.');
      return;
    }
    if (!size) {
    ...
    try {
      ...
      addLog(`${syncedOrders.length} order(s) sync hua.`, 'success');
    } catch (err) {
    ...
    if (syncedOrders.length === 0) {
      addLog('Koi order sync nahi hua, scan skip.', 'warn');
      toast.warn('Koi order sync nahi hua.');
    ...
    for (const order of syncedOrders) {
      ...
      if (!orderId) {
        addLog(`Order ID nahi mila: ${JSON.stringify(order).slice(0, 60)}`, 'warn');
        continue;
      }
      ...
      try {
        ...
        addLog(`Order #${orderId} scan ho gaya.`, 'success');
      } catch (err) {
    ...
    setStatus(STATUS.DONE);
    addLog(
      `Complete! ${scanned}/${syncedOrders.length} orders scan hue.`,
      allOk ? 'success' : 'warn'
    );

    if (allOk) {
      toast.success(`✓ ${scanned} order(s) scan ho gaye! Form ${AUTO_CLEAR_SECONDS}s mein clear hoga.`);
    } else {
      toast.warn(`${scanned}/${syncedOrders.length} orders scan hue. Kuch fail hue.`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 font-sans">
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4 pb-24">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <h2 className="text-lg font-bold text-gray-900 mb-5" style={{ fontFamily: 'Nunito, sans-serif' }}>
            Missing Piece Scan
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Size
                {size && (
                  <span className="ml-2 text-indigo-600 font-semibold">— {size}</span>
                )}
              </label>
              ...
              <>
                <div className="flex gap-3 mb-4">
                  ...
                  <div className="flex-1 bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-center">
                    <p className="text-2xl font-bold text-indigo-600" style={{ fontFamily: 'Nunito, sans-serif' }}>
                      {scannedCount}
                    </p>
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto">
              {logs.map((log, i) => (
                <div
                  ...
                  `}
                >
                  <span className="text-xs opacity-40 mt-0.5 shrink-0 tabular-nums">{log.time}</span>
